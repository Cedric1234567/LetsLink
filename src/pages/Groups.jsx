    ...
    <div className="page">
      ...
      {mode !== 'group' && (
        <div style={{ margin: '0 20px 12px', background: 'var(--primary-light)', borderRadius: 14, padding: '12px 14px', fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>
          You are currently in Solo mode. You can still view groups, or switch mode from Home before planning as a group.
        </div>
      )}
      ...
      {groups.length === 0 ? (
        ...
        groups.map(group => {
          const currentCount = Array.isArray(group.memberNames) && group.memberNames.length > 0
            ? group.memberNames.length
            : (group.currentMembers || 0)
          const capacity = group.maxMembers || currentCount
          return (
              ...
              <div className="group-card-info">
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
                    {currentCount} of {capacity} members
                  </span>
                </div>
              </div>
            </div>
